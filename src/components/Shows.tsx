...
const Shows = () => {
    ...
    const [APIShows, setAPIShows] = React.useState<apiMedia[]>([]);
    const [fullShowData, setFullShowData] = React.useState<apiMediaFull[]>([]);
    ...
    React.useEffect(() => {
        setFullShowData([])
        axios.get(`https://vidsrc.to/vapi/tv/new/${page}`)
            .then((response) => {
                setAPIShows(response.data.result.items)
            })
            .catch((error) => {
                setErrorFlag(true);
    ...
    React.useEffect(() => {
        const fetchFullShowData = async () => {
            setFullShowData(await getFullData(APIShows))
        }
    ...
    const list_of_films = () => {
        ...
        return (mediaCardGeneration(fullShowData));
    };

    if (fullShowData.length == 0) {
        return (
            <div className="WatchShowsBody">
                <Header></Header>
                <h1>Shows</h1>
            ...
            <div className="WatchShowsBody">
                ...
                <div>
                    ...
                    <div className="container mt-4 ml-5 mr-5">
                        {APIShows.length < 5 &&
                            <div className={`row row-cols-1 row-cols-md-${APIShows.length}`}>
                                {list_of_films()}
                            </div>
                        }
                        {APIShows.length >= 5 &&
